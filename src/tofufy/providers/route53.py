...
from tofufy.render import render_assignment_map, render_attribute_block, replace_marked_block_in_file
...
def configure_parser(parser: argparse.ArgumentParser, normalized_config: Dict[str, Any]) -> None:
    parser.add_argument(
        "--only-hostnames",
        help="Comma-separated list of fully-qualified hostnames; only matching records will be exported.",
    )
    parser.add_argument(
        "--export-target",
...
def get_zone_details(client, zone_id: str, include_tags: bool = True) -> Dict[str, Any]:
    ...
    for vpc in response.get("VPCs") or []:
        ...
        if not vpc_id:
            continue
        vpcs.append({"vpc_id": vpc_id, "vpc_region": vpc.get("VPCRegion")})
...
def build_record_key(record_name: str, zone_name: str, record_type: str) -> Tuple[str, str, str]:
    ...
    if (relative_name and relative_name.startswith("_")) or (subdomain and subdomain[0].isdigit()):
...
def normalize_record(record: dict, zone_name: str, zone_id: str) -> Optional[Dict[str, Any]]:
    ...
    if resource_records:
        ...
        for entry in resource_records:
            ...
            values.append(escape_percent_signs(value))
...
def build_record_attributes(record: Dict[str, Any]) -> "OrderedDict[str, Any]":
    ...
    for original_key, target_key in {
...
def render_zone_file(
    ...
    lines: List[str] = ["locals {", f"  {local_var} = {{", f"    {json.dumps(zone_key)} = {{"]
    if records:
...
def render_single_zone_records(
    ...
    return render_assignment_map(local_var, records, map_indent=2, item_indent=3)
...
def build_vpc_map(vpcs: List[Dict[str, Any]]) -> "OrderedDict[str, OrderedDict[str, Any]]":
    ...
    sorted_vpcs = sorted(vpcs, key=lambda item: (item.get("vpc_region") or "", item.get("vpc_id") or ""))
    for index, vpc in enumerate(sorted_vpcs, start=1):
        identifier = sanitize_identifier(f"{vpc.get('vpc_region') or 'unknown'}_{vpc.get('vpc_id') or index}")
        if not identifier:
...
def build_zone_configuration(
    ...
    tags: Dict[str, str] = zone_details.get("tags") or {} if include_tags else {}
...
def collect_zone_records(
    ...
    filename_domain = f"{zone_name.replace('.', '-')}{'-private' if private_zone else ''}"
    ...
    for record in sorted(records, key=lambda item: (item["key_base"], item.get("set_identifier") or "", item["full_name"])):
        ...
        record_blocks[record_key] = build_record_attributes({**record, "key": record_key})
...
def export_records(
    ...
    zone_key, local_var, filename_domain, record_blocks, import_entries = collect_zone_records(
        client, zone_id, zone_name, private_zone, skip_patterns, include_patterns
    )
...
def write_single_zone_records(
    ...
    records_path.write_text(render_single_zone_records(local_var, record_blocks), encoding="utf-8")
...
def write_imports_file(
    ...
    lines: List[str] = []
    if zone_resource_id:
        lines.extend(["import {", "  to = module.zone.aws_route53_zone.this[0]", f"  id = {json.dumps(zone_resource_id)}", "}", ""])
    for zone_key, zone_id in zone_import_entries:
        lines.extend(["import {", f"  to = module.zones[{json.dumps(zone_key)}].aws_route53_zone.this[0]", f"  id = {json.dumps(zone_id)}", "}", ""])
    for zone_key, record_key, import_id in import_entries:
        to_line = (
            f"  to = module.zone.aws_route53_record.this[{json.dumps(record_key)}]"
            if single_zone
            else f"  to = module.zones[{json.dumps(zone_key)}].aws_route53_record.this[{json.dumps(record_key)}]"
        )
        lines.extend(["import {", to_line, f"  id = {json.dumps(import_id)}", "}", ""])
...
def update_single_zone_locals(
    ...
    replace_marked_block_in_file(locals_path, SINGLE_ZONE_BEGIN_MARKER, SINGLE_ZONE_END_MARKER, "\n".join(zone_block))


def update_locals_file(local_vars: List[str], locals_path: Path) -> None:
    block_lines: List[str] = [BEGIN_MARKER]
    if local_vars:
        block_lines.extend(["locals {", "  zone_records = merge(", "    {},"])
        ...
        block_lines.extend(["locals {", "  zone_records = {}", "}"])
    block_lines.extend([END_MARKER, ""])
    replace_marked_block_in_file(locals_path, BEGIN_MARKER, END_MARKER, "\n".join(block_lines))
...
def parse_hostname_patterns(raw: Optional[Union[str, Iterable[str]]]) -> List[Pattern[str]]:
    if not raw:
        return []
    values = [raw] if isinstance(raw, str) else [str(item) for item in raw if item]
...
def run(args: argparse.Namespace) -> int:
    ...
    export_records_enabled = args.export_target in ("records", "both")
    export_zones_enabled = args.export_target in ("zones", "both")
    ...
    if single_zone_mode:
        records_path = resolve_output_path(args.single_zone_records_file)
        locals_path = resolve_output_path(args.locals_file)
        zone_id = zone_ids[0]
        ...
        try:
            ...
            if export_records_enabled:
                _zone_key, _local_var, _filename, record_blocks, import_entries = collect_zone_records(
                    client, zone_id, zone_name, private_zone, skip_patterns=skip_patterns, include_patterns=include_patterns
                )
            ...
            if export_zones_enabled:
                update_single_zone_locals(zone_details, locals_path, include_tags=not skip_zone_tags)
                zone_resource_import_id = zone_id
                per_zone_messages.append(f"zone metadata -> {locals_path}")
        except Exception as exc:
        ...
        locals_path = resolve_output_path(args.locals_file)
        zones_path = resolve_output_path(args.zones_file)
        if export_records_enabled:
            output_dir.mkdir(parents=True, exist_ok=True)
        ...
        for zone_id in zone_ids:
            try:
                ...
                per_zone_messages: List[str] = []
                if export_records_enabled:
                    zone_key_from_records, local_var, output_path, import_entries = export_records(
                        client, zone_id, zone_name, private_zone, output_dir, skip_patterns=skip_patterns, include_patterns=include_patterns
                    )
                ...
                if zone_export_enabled:
                    ...
                    aggregate_zone_configs[zone_key] = build_zone_configuration(zone_details, include_tags=not skip_zone_tags)
        ...
        if export_records_enabled:
            try:
                update_locals_file(sorted(set(aggregate_locals)), locals_path)
                print(f"Updated {locals_path} with generated zone record locals")
            except OSError as exc:
                print(f"Failed to update locals file {locals_path}: {exc}", file=sys.stderr)
                return 1
        if zone_export_enabled:
            ...
            if aggregate_zone_configs:
                ...
                try:
                    write_zones_file(ordered_zones, zones_path)
                    print(f"Wrote zone configuration to {zones_path}")
                except OSError as exc:
                    print(f"Failed to write zones file {zones_path}: {exc}", file=sys.stderr)
                    return 1
            else:
                print("No zone configurations were generated; skipping zones file update.", file=sys.stderr)

    aggregate_imports = sorted(set(aggregate_imports))
    imports_path = resolve_output_path(args.imports_file)
    try:
        write_imports_file(
            aggregate_imports,
            imports_path,
        ...
        print(f"Wrote import statements to {imports_path}")
    except OSError as exc:
        print(f"Failed to write imports file {imports_path}: {exc}", file=sys.stderr)
        return 1

    if successes != len(zone_ids):
        print(f"Completed with partial success: {successes}/{len(zone_ids)} zones exported.", file=sys.stderr)
        return 2
    ...
    if export_records_enabled:
        ...
        destination_parts.append(f"records -> {args.single_zone_records_file if single_zone_mode else args.output_dir}")
    if zone_export_enabled:
